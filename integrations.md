...
Integrations construct `Boundary` + `Identity`, call `gate.check`/`gate.restore`, and translate `PolicyViolationError` into the host framework's native failure mode. They contain zero policy logic.

## 1. @gate/ai-sdk — Vercel AI SDK middleware (flagship)

Public API:

```ts
gate.model(model: LanguageModel, opts?: { agent?: string; workflowId?: string | (() => string) }): LanguageModel
gate.tools<T extends ToolSet>(tools: T, opts?: { agent?: string; workflowId?: string }): T
```

`gate.model` wraps via the AI SDK's `wrapLanguageModel` middleware API:
...
`gate.tools` wraps each tool's `execute`: `check` args at `{ kind: "tool", name, direction: "out" }`, `check` the return value at `direction: "in"`. Blocked ⇒ throw; the AI SDK surfaces tool errors to the model, which is the desired self-correction loop. Preserve the tool's type signature exactly (generics, no `any`).
...
## 2. @gate/mcp — MCP client wrapper

```ts
gate.mcp<T extends Transport>(transport: T, opts: { server: string; agent?: string; workflowId?: string }): T
```
...
## 3. @gate/hono — middleware

```ts
app.use("/v1/*", gateHono(gate, { mode: "openai-compatible", agent: (c) => c.req.header("x-agent-id") ?? "default" }))
```
...
## 4. @gate/cli — `gate` binary

Commands:
- `gate init` — detect stack (presence of `ai`, `hono`, `next` in package.json), write `gate.config.ts` from a template with the default policy inlined and commented, print the 3-line integration snippet for the detected stack.
- `gate scan <path|-> [--json]` — run Tier 0 over files or stdin; exit 1 if any `block`-class entity found. (Free utility value: works as a pre-commit secret scanner; mention in README.)
- `gate install-hooks [--scope project|user]` — merge our hook entries into `.claude/settings.json` (project) or `~/.claude/settings.json` (user). NEVER overwrite existing hooks: parse, append to the `PreToolUse`/`PostToolUse` matcher arrays, preserve formatting where feasible, back up the original file first. Print what was added.
- `gate hook` — the hook handler itself (see below).

### Claude Code hook handler contract (`gate hook`)

- Reads the event JSON from stdin (fields include `hook_event_name`, `tool_name`, `tool_input`).
- Loads `gate.config.ts` from `$CLAUDE_PROJECT_DIR` (esbuild-register or precompiled `gate.config.json` fallback; document the tradeoff, prefer compiling config to JSON at `install-hooks` time and recompiling on change for speed).
...
- Every integration accepts an optional `onDecision(decisions: Decision[])` callback and forwards to the gate's audit emitter regardless.

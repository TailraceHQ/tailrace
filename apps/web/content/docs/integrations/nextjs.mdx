...
See [Ship an agent on Vercel](/docs/guides/ship-an-agent-on-vercel#deploy-to-vercel) for env vars (`OPENAI_API_KEY`, `TAILRACE_VAULT_KEY`), `vercel` deploy, and production curl checks against `/api/chat`.

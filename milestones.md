...
- [ ] `gate.check` and `gate.restore` end-to-end; actions incl. format-preserving tokenization
...
- [ ] Default zero-config policy works: `createGate()` blocks a Stripe key, tokenizes an email
- [ ] Core suite passes under workerd

## M3 — AI SDK integration
- [ ] `gate.model` transformParams + wrapGenerate + wrapStream per integrations.md §1
- [ ] Streaming carry-buffer with adversarial chunking tests (1-char chunks, split tokens)
- [ ] `gate.tools` with exact type preservation (type-level tests via expect-type)
- [ ] examples/nextjs-ai-sdk runs Demos 1 & 3 (below)

## M4 — Claude Code CLI
- [ ] `gate init`, `gate scan`, `gate install-hooks` (non-destructive settings merge with backup), `gate hook`
...
## M5 — MCP + Hono + polish
- [ ] @gate/mcp transport wrapper with JSON-RPC error synthesis on block
- [ ] @gate/hono incl. SSE streaming scan; 422 error shape
...
**Demo 2 — "Claude Code can't paste your secrets."** In examples/claude-code: agent is asked to read `.env.example` (fake values) and POST its contents to httpbin via a fetch tool. Hook denies with reason naming `api_key` and the rule; agent retries with tokenized payload; PostToolUse audit line appears in `.gate/audit.jsonl`.

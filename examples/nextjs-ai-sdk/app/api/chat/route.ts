...
const tailrace = withAiSdk(
  createTailrace({
    vault: { key: process.env["TAILRACE_VAULT_KEY"] ?? "demo-vault-key-not-for-prod" },
  }),
);
...
export async function POST(req: Request): Promise<Response> {
  ...
  const model = tailrace.model(createEchoModel(), {
    workflowId,
    agent: "demo-1",
  ...
  try {
    ...
    return Response.json({
      ...
      workflowId,
      // Transformed (tokenized) text as seen by the mock model before restore.

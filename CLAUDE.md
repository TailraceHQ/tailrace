...
**Read `AGENTS.md` first.** It is the master build prompt and links every spec. The spec docs are normative: when a spec and your instincts disagree, the spec wins. Note: `AGENTS.md` refers to the specs as living in `docs/`, but they currently sit at the repo root (`architecture.md`, `policy-engine.md`, `detection.md`, `vault.md`, `integrations.md`, `milestones.md`, `conventions.md`). When the monorepo is scaffolded, they move to `docs/`.

## Naming

Product name is TBD. Use the npm scope `@gate/*` and CLI name `gate` as placeholders everywhere. Never invent an alternative name.
...
2. **TypeScript only, strict mode.** Node >= 20; `@gate/core` must also run on Cloudflare Workers and Vercel Edge (no `node:` imports, no `Buffer`, WebCrypto only, no filesystem).
...
5. **Zero required config.** `createGate()` with no args enforces the default policy (secrets → block, common PII → tokenize).
...
- `packages/core` (`@gate/core`) - detection, policy engine, vault, audit. **Zero runtime dependencies.** Public exports are limited to: `createGate`, `definePolicy`, `defineRecognizer`, `memoryVault`, `kvVault`, `staticPolicy`, error classes, and public types.
- `packages/ai-sdk`, `packages/mcp`, `packages/hono`, `packages/cli` - integrations; depend on `core` plus their host framework as a **peer dependency**. Integrations contain zero policy logic: they construct `Boundary`/`Identity`, call `gate.check`/`gate.restore`, and translate `PolicyViolationError` into the host's failure mode.
...
- All errors extend `GateError { code }`; error messages never contain detected values (enforced by a test).
